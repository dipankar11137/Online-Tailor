...
import ManageTailor from './ManageTailor';
...
const ManageTailors = () => {
  ...
  const handleAdd = () => {
    navigator('/dashboard/addTailors');
  };
  const handleDelete = id => {
    const proceed = window.confirm('Are You Sure ?');
    if (proceed) {
      const url = `http://localhost:5000/tailors/${id}`;
      fetch(url, {
        method: 'DELETE',
      })
        .then(res => res.json())
        ...
        <table className="table-auto w-full bg-white rounded-lg">
          ...
          <thead className="bg-gray-800 text-white">
            <tr className="text-center">
              ...
              <th className="text-lg font-semibold">Salary</th>
